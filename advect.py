...
adv_params = {'a':3, 'σ':0.7}

nxs = np.arange(10,2100,100)
...
for i, nx in enumerate(nxs):
    ...
    err[1,i] = LA.norm(model.U - analytical,np.inf)

    model = advection(params,adv_params)
    model.U[0,:] = 5*np.sin((model.x-np.pi/2))+5
    model.run('FTCS')
    err[0,i] = LA.norm(model.U - analytical,np.inf)

    model = advection(params,adv_params)
    model.U[0,:] = 5*np.sin((model.x-np.pi/2))+5
    model.run('LaxWendroff')
    err[2,i] = LA.norm(model.U - analytical,np.inf)

    model = advection(params,adv_params)
    model.U[0,:] = 5*np.sin((model.x-np.pi/2))+5
    model.run('BeamWarming')
    err[3,i] = LA.norm(model.U - analytical,np.inf)

fig, ax = plt.subplots(1,1,figsize=(6,4))
# ax.set_yscale('log')
# ax.set_xscale('log')
ax.loglog(1/nxs,err[0],'kx-',label='FTCS',   markersize=4)
ax.loglog(1/nxs,err[1],'ko-',label='UpWind', markersize=3)
ax.loglog(1/nxs,err[2],'k-.',label='LaxWendroff')
ax.loglog(1/nxs,err[3],'k:',label='BeamWarming')
ax.grid()
